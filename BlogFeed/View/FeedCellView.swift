...
struct FeedCellView: View {
    ...
    var body: some View {
        VStack{
            ...
            Divider().padding(18)
            Group{
                Text("First Name: ").font(.headline).foregroundColor(.cyan).fontWidth(.condensed)
                Text("\(feed.user.firstName)").foregroundColor(.indigo)
                Text("User Name: ").font(.headline).foregroundColor(.cyan).fontWidth(.condensed)
                Text("\(feed.user.username)").foregroundColor(.indigo)
                Text("Total Likes: ").font(.headline).foregroundColor(.cyan).fontWidth(.condensed)
                Text("\(feed.user.totalLikes)").foregroundColor(.indigo)
                Text("Location: ").font(.headline).foregroundColor(.cyan).fontWidth(.condensed)
                Text("\(feed.user.location ?? "Location Not Available")")
                Text("Created: ").font(.headline).foregroundColor(.cyan).fontWidth(.condensed)
